...
server_microtcp (uint16_t listen_port, const char *file)
{
  ...
  memset(&(sock.peer_addr), 0, sizeof(address));

  microtcp_accept (&sock, (struct sockaddr*)&(sock.peer_addr), sizeof(sock.peer_addr));
  
  buffer=malloc(MICROTCP_RECVBUF_LEN);


  address_ln=sizeof(address);


  clock_gettime(CLOCK_MONOTONIC_RAW, &start_time);
  while((received = microtcp_recv(&sock, buffer, MICROTCP_RECVBUF_LEN, MSG_WAITALL))!=-1){ 
    if(received==-1){
...
client_microtcp (const char *serverip, uint16_t server_port, const char *file)
{
  ...
  while (!feof (fp)) {
    ...
    data_sent = microtcp_send(&sock, buffer, read_items*sizeof(uint8_t)  , MSG_CONFIRM);
    // printf("Data sent: %d  read_items * sizeof(uint8_t) %d\n", data_sent, read_items * sizeof(uint8_t));
    if (data_sent != read_items * sizeof(uint8_t)) {
      printf ("Failed to send the"
              " amount of data read from the file.\n");
      microtcp_shutdown (&sock, SHUT_RDWR);
